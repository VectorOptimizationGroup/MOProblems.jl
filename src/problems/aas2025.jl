...
# ------------------------- AAS3 -------------------------
"""
    AAS3(; T::Type{<:AbstractFloat}=Float64,
         p1=1.003, λ1=1.2, Φ1=[1.2 -0.3; 0.4 1.5], c1=[1.5; -1.0],
         p2=1.002, λ2=0.8, Φ2=[1.8 0.5; -0.2 1.1], c2=[-1.2; 0.8],
         A3=[2.0 0.5; 0.5 1.5], b3=[1.0; -0.5])

A function from R² to R³ with two objectives with Hölder continuous gradients
and one objective with Lipschitz continuous gradient.

The domain is the square [-10, 10] x [-10, 10].
...
- `Φ2`: Linear transformation matrix for f₂.
- `c2`: Center for f₂.
- `A3`: Positive definite matrix for f₃.
- `b3`: Vector for f₃.
"""
function AAS3(; T::Type{<:AbstractFloat}=Float64,
    p1=1.003,
    λ1=1.2,
    Φ1=[1.2 -0.3; 0.4 1.5],
    c1=[1.5; -1.0],
    ...
    @assert size(Φ1) == (2, 2) "Φ1 deve ser uma matriz 2x2"
    @assert size(Φ2) == (2, 2) "Φ2 deve ser uma matriz 2x2"
    @assert size(A3) == (2, 2) "A3 deve ser uma matriz 2x2"
    @assert length(c1) == 2 "c1 deve ser um vetor de dimensão 2"
    @assert length(c2) == 2 "c2 deve ser um vetor de dimensão 2"
    ...
    grad_f3 = x -> A3_T' * (A3_T * x - b3_T)

    jacobian = x -> [grad_f1(x)'; grad_f2(x)'; grad_f3(x)']
